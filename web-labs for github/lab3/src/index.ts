    ...
    borrowButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (!book.borrowerId) {
                ...
                renderBooks();
                renderUsers();
                showNotification(`Книга "${book.title}" успішно позичена користувачем ${user.name}.`, 'success');
            } else {
                ...
                renderBooks();
                renderUsers();
                showNotification(`Книга "${book.title}" успішно повернута користувачем ${user.name}.`, 'success');
            }
        });
    });
}
...
bookForm.addEventListener('submit', (e) => {
    ...
    if (valid) {
        ...
        const newBook = new Book(Date.now(), titleInput.value, authorInput.value, parseInt(yearInput.value), undefined);
